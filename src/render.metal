...
kernel void render(device uint8_t* data, device float* zbuffer, device const float* vertices, device const float* colors, device uint32_t* faces, const device uint* Nf, const device uint* Nv, uint2 index [[thread_position_in_grid]]) {
    int nf = index.x;
